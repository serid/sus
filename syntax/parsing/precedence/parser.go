...
func (parser Parser) ParseE(lexemes []lexeme.Lexeme) (interface{}, error) {
	...
	if len(outStack) > 1 {
		return nil, errors.TrailingLexemes{}
...
func (parser Parser) Parse(lexemes []lexeme.Lexeme) interface{} {
	r, err := parser.ParseE(lexemes)
	stuff.Unwrap(err)
	return r
}
...
func reduce(oper lexeme.Lexeme, outStack *OutStack) error {
	if len(*outStack) >= 2 {
		...
		*outStack = (*outStack)[:len(*outStack)-1]
		...
		return errors.NewEof(lexeme.Any())
	} else {
		// len(*outStack) == 0
		// Reached EOF
		return errors.NewEof(lexeme.Any())
